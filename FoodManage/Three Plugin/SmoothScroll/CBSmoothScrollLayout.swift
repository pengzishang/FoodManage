...
class CBSmoothScrollLayout: UICollectionViewLayout {
    ...
    public var headerMinHeight: CGFloat = 200 {
        didSet { resetStoredValues() }
    }
    public var menuMaxHeight: CGFloat = 100 {
        didSet { resetStoredValues() }
    }
    public var menuMinHeight: CGFloat = 60  {
        didSet { resetStoredValues() }
    }
    ...
    public var menuToHeaderEndOffset: CGFloat = -30  {
        didSet { resetStoredValues() }
    }
    ...
    public var titleStartOffset: CGPoint = CGPoint(x: 50.0, y: 30)  { //Offset to menu top
        didSet { resetStoredValues() }
    }
    public var titleEndOffset: CGPoint = CGPoint(x: 50.0, y: 80)  { //Offset to header top
        didSet { resetStoredValues() }
    }
    
    
    private var defaultCellZIndex: Int = 5
    public var animationScrollLength: CGFloat = 100.0  {
        didSet {
    ...
    private func headerSize(forProgress progress: CGFloat) -> CGFloat {
        ...
        if UIApplication.shared.statusBarFrame.height > 20 {
            return headerMaxHeight - (headerMaxHeight - headerMinHeight) * progress + 24
        }
    ...
    private func menuFrame(forProgress progress: CGFloat) -> CGRect {
        ...
        var menuY = maxY - (maxY - minY) * progress
        if UIApplication.shared.statusBarFrame.height > 20 {
            menuY += 24
        }
        ...
        if offset <= 0 {
            ...
            if UIApplication.shared.statusBarFrame.height > 20 {
                return CGPoint(x: startPoint.x + (endPoint.x - startPoint.x) * animationProgress,
                               y: startPoint.y + (endPoint.y - startPoint.y) * animationProgress + 24)
            }
            return CGPoint(x: startPoint.x + (endPoint.x - startPoint.x) * animationProgress,
